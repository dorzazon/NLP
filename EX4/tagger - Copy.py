...
def baseline_tag_sentence(sentence, perWordTagCounts, allTagCounts):
    ...
    for word in sentence:
        if word not in [key[0] for key, value in perWordTagCounts.items()]:
...
class BiLSTM_Tagger(nn.Module):
    def __init__(self, embedding_dimension, vectors, num_of_layers=1, hidden_size=64, output_dimension=18):
        super().__init__()
...
class CBiLSTM_Tagger(nn.Module):
    def __init__(self, embedding_dimension, vectors, num_of_layers=1, hidden_size=64, output_dimension=18):
        ...
        self.lstm = nn.LSTM(input_size=embedding_dimension + 3, hidden_size=hidden_size, num_layers=num_of_layers,
                            batch_first=True, bidirectional=True)
        self.hidden2tag = nn.Linear(hidden_size*2, output_dimension)
...
def initialize_rnn_model(params_d):
    ...
    # build vocabulary
    Sentences = data.Field(batch_first=True, pad_first=True, unk_token=UNK)
    ...
    if params_d['input_rep'] == 0:
        model = BiLSTM_Tagger(embedding_dimension=params_d['embedding_dimension'], output_dimension=params_d['output_dimension'] + 1,
                              vectors=vectors, num_of_layers=params_d['num_of_layers'])
    else:
        model = CBiLSTM_Tagger(embedding_dimension=params_d['embedding_dimension'], output_dimension=params_d['output_dimension'] + 1,
                               vectors=vectors, num_of_layers=params_d['num_of_layers'])
...
def get_best_performing_model_params():
    ...
    """
    return {'max_vocab_size': -1, 'min_frequency': 3, 'input_rep': 1, 'embedding_dimension': 100, 'num_of_layers': 1,
            'output_dimension': 17}
def rnn_tag_sentence(sentence, model, input_rep = 0):
    """ Returns a list of pairs (w,t) where each w corresponds to a word
           (same index) in the input sentence.
       ...
       Return:
           list: list of pairs
       """
    dataset = build_test_dataset(sentence, model.stoi, input_rep)
    loader = data.BucketIterator(dataset, batch_size=1)
    for s in loader:
        if input_rep == 0:
            predictions = model(s.Sentences)
        else:
            predictions = model(s.Sentences, torch.tensor(s.Cases))
    labels_idx = np.argmax(predictions.detach().numpy(), axis=1)
    tagged_sentence = build_tags(labels_idx[0], sentence, model.tag_itos)
    return tagged_sentence
#==========================================================
...
def count_correct(gold_sentence, pred_sentence):
    ...
    """
    OOV = sum([w[0] not in voc for w in gold_sentence])
    correct = sum([g[1] == p[1] for g, p in zip(gold_sentence, pred_sentence)])
    correctOOV = sum([g[1] == p[1] for g, p in zip(gold_sentence, pred_sentence) if g[0] not in voc])
...
def build_test_dataset(test_sentence, stoi, input_rep):
    Sentences = data.Field(use_vocab=False, batch_first=True, pad_first=True, unk_token=UNK)
    test_example = []
